...
export class BrowserStorageService extends StorageService {
    ...
    switch (this.storageType) {
      ...
      case StorageType.SESSION_COOKIE:
        this.setCookie(
          key,
          value,
          this.storageType === StorageType.COOKIE ? date : ''
        );
        break;
      case StorageType.LOCAL_STORAGE:
      case StorageType.SESSION_STORAGE:
  ...
  private isWindowStorageAvailable(
    storageType: StorageType.SESSION_STORAGE | StorageType.LOCAL_STORAGE
  ) {
    try {
      const supported =
        window && storageType in window && window[storageType] !== null;
    ...
    try {
      ...
      if (supported) {
        ...
        this.setCookie(
          key,
          'test',
          new Date(Date.now() + 60 * 1000).toUTCString()
        );
