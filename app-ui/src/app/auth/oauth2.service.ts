...
export class Oauth2Service implements IOauthService {
  ...
  open<T extends object | string = any>(
    ...
    return this.popup.open(url, oauthOptions, this.config.options.cordova).pipe(
      ...
      switchMap((oauthData: any) => {
        ...
        return this.exchangeForToken<T>(
          oauthOptions,
          authorizationData,
          oauthData,
          userData
  ...
  private exchangeForToken<T>(
    ...
    return this.http.request<T>(method, exchangeForTokenUrl, {
      body,
      withCredentials
    });
  }
  ...
  private getAuthorizationData(options: IOauth2Options) {
    ...
    return [
      ...
      ...(additionalUrlParams
        ? Object.keys(additionalUrlParams).map(key => {
            const value:
              ...
              | (() => string)
              | null
              | undefined = (additionalUrlParams as any)[key];
      ...
      .filter(_ => !!_[0])
      .reduce((acc, next) => ({ ...acc, [next[0]]: next[1] }), {} as {
        [key: string]: string;
      });
  }
}
